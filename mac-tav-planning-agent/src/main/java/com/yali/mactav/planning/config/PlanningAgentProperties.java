...
/**
 * Configuration properties for the real MAC-TAV PlanningAgent wrapper.
 *
 * <p>The properties belong to mac-tav-planning-agent and configure only local
 * Agent behavior. They must not contain API keys or Orchestrator/Web wiring.</p>
 */
@ConfigurationProperties(prefix = "mactav.agents.planning")
public class PlanningAgentProperties {
    ...
    private String promptPath = "prompts/planning-agent-prompt.md";
    ...
    public String effectivePromptPath() {
        return promptPath == null || promptPath.isBlank() ? "prompts/planning-agent-prompt.md" : promptPath;
    }
}
