...
- 返回 `taskId`、`taskStatus`、`currentStage`、`createdAt`。
...
执行必须通过 Orchestrator / ExecutionAdapter 和白名单 Adapter。API 不接收任意 shell 命令。
...
- Controller 不直接调用 VerificationAgent。
...
以下路径可作为早期 Demo 或兼容理解，但不作为长期推荐 API 设计：

- `/api/v1/tasks/{taskId}/workspace`
- `/api/v1/tasks/{taskId}/artifacts`
- `/api/v1/tasks/{taskId}/execution`
- `/api/v1/tasks/{taskId}/validation`
- `/api/v1/tasks/{taskId}/repair`
...
| 早期路径 | 长期推荐路径 |
| --- | --- |
| `/api/v1/tasks/{taskId}/workspace` | `/api/v1/workspaces/{taskId}` |
| `/api/v1/tasks/{taskId}/timeline` | `/api/v1/workspaces/{taskId}/timeline` |
| `/api/v1/tasks/{taskId}/artifacts` | `/api/v1/artifacts?taskId={taskId}` |
| `/api/v1/tasks/{taskId}/run` | `/api/v1/workflows/{taskId}/run` |
| `/api/v1/tasks/{taskId}/stages/{stage}/rerun` | `/api/v1/workflows/{taskId}/stages/{stage}/rerun` |
| `/api/v1/tasks/{taskId}/execution` | `/api/v1/executions/{taskId}` |
| `/api/v1/tasks/{taskId}/validation` | `/api/v1/validations/{taskId}` |
| `/api/v1/tasks/{taskId}/repair` | `/api/v1/repairs/{taskId}` |
| `/api/v1/tasks/{taskId}/views/topology` | `/api/v1/views/{taskId}/topology` |
| `/api/v1/tasks/{taskId}/events` | `/api/v1/events/{taskId}` |

不要把早期路径和长期路径写成两套长期 API。长期推荐只有资源域拆分后的 API。
